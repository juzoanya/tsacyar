...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   check_valid_scene_utils.c                          :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: juzoanya <juzoanya@student.42wolfsburg.    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/10/21 17:06:10 by euyi              #+#    #+#             */
/*   Updated: 2022/10/29 18:16:23 by juzoanya         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	scene_is_valid(t_cube *cube, char **rawdata)
{
	...
	if (i == 6 && no_wrong_element(rawdata, max_arr_pos) && map_is_valid(cube, rawdata + max_arr_pos + 1)) 
		return (1);
	return (0);
}
...
int	check_for(void *dest, char **rawdata, char *ptr, int *arr_pos)
{
	...
	if ((flag == 2 && verify_path((char *)dest, rawdata[i] + exist_in_start(rawdata[i], ptr)))
		|| (flag == 1 && verify_rgb((int *)dest, rawdata[i] + exist_in_start(rawdata[i], ptr))))
		return (1);
	return (0);
}
